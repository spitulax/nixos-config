# Taken from https://github.com/NixOS/nixpkgs/blob/dc9637876d0dcc8c9e5e22986b857632effeb727/nixos/modules/services/databases/mysql.nix
...
let
  ...
  cfg = config.services.mysqlNoAutostart;
  ...
  generateClusterAddressExpr = ''
    if (config.services.mysqlNoAutostart.galeraCluster.nodeAddresses == [ ]) then
      ""
    else
      "gcomm://''${builtins.concatStringsSep \",\" config.services.mysqlNoAutostart.galeraCluster.nodeAddresses}"
      + lib.optionalString (config.services.mysqlNoAutostart.galeraCluster.clusterPassword != "")
        "?gmcast.seg=1:''${config.services.mysqlNoAutostart.galeraCluster.clusterPassword}"
  '';
...

{
  imports = [
    ...
    (lib.mkRemovedOptionModule [
      ...
      "mysqlNoAutostart"
      "extraOptions"
    ] "Use services.mysqlNoAutostart.settings.mysqld instead.")
    (lib.mkRemovedOptionModule [
      "services"
      "mysqlNoAutostart"
      "bind"
    ] "Use services.mysqlNoAutostart.settings.mysqld.bind-address instead.")
    (lib.mkRemovedOptionModule [
      "services"
      "mysqlNoAutostart"
      "port"
    ] "Use services.mysqlNoAutostart.settings.mysqld.port instead.")
  ];
    ...
    services.mysqlNoAutostart = {
      ...
      configFile = lib.mkOption {
        ...
        description = ''
          ...
          NixOS generates one automatically from {option}`services.mysqlNoAutostart.settings`.
        '';
      ...
      galeraCluster = {
        ...
        localAddress = lib.mkOption {
          ...
          defaultText = lib.literalExpression "config.services.mysqlNoAutostart.galeraCluster.localName";
        };
  ...
  config = lib.mkIf cfg.enable {
    assertions = [
      {
        ...
        message = "'services.mysqlNoAutostart.galeraCluster.enable' expect services.mysqlNoAutostart.package to be an mariadb variant";
      }
    ]
    ...
    ++ lib.optionals cfg.galeraCluster.enable [
      ...
      }
      {
        ...
        message = "When services.mysqlNoAutostart.galeraCluster.clusterAddress is set, setting services.mysqlNoAutostart.galeraCluster.nodeAddresses is redundant and will be overwritten by clusterAddress. Choose one approach.";
      }
      {
        assertion = !config.services.mysql.enable;
        message = "services.mysql and services.mysqlNoAutostart are mutually exclusive";
      }
    ];

    services.mysqlNoAutostart.dataDir = lib.mkDefault (
      if lib.versionAtLeast config.system.stateVersion "17.09" then "/var/lib/mysql" else "/var/mysql"
    );

    services.mysqlNoAutostart.settings.mysqld = lib.mkMerge [
      {
        datadir = cfg.dataDir;
    ...
    services.mysqlNoAutostart.settings.galera = lib.optionalAttrs cfg.galeraCluster.enable {
      wsrep_on = "ON";
